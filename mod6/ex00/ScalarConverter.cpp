...
{
    try                         { return stod(s) == (double)stoi(s); }
...
static type getType(const std::string& s)
{
    ...
    if (isType(s) == false) return NONE;
    if (isChar(s) == true)  return CHAR;
    if (isFloat(s) == true) return FLOAT;
    if (isInt(s) == true)   return INT;
    return DOUBLE;
}
...
void ScalarConverter::convert(const std::string& s)
{
    ...
    else if (d - 1 >= INT_MAX || d + 1 <= INT_MIN) std::cout << "overflow" << std::endl;
